...
export const generatePrismaSchema = (models: ModelDefinition[], schemaPath?: string) => {
  ...
  for (const _model of models) {
    ...
    for (const _field of _model.fields) {
      ...
      prismaFields.push(`  ${_field.name}${blankSpacesAfterProperty}${mappedType}${_field.isNullable ? '?' : ''}${blankSpacesAfterType}${prismaMap}  ${_field.isPrimary ? prismaPrimary : _field.isUnique ? prismaUnique : ''}`)
    }
